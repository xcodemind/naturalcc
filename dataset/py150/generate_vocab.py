...
from dataset.py150.utils import file_tqdm, get_dfs
import os
...
def main():
    ...
    parser.add_argument("--n_vocab", "-n", type=int, default=10000)
    ...
    vocab_to_keep_frequency = [i[1] for i in vocab.most_common(args.n_vocab)]
    ...
    with open(args.out_fp, "w") as fout:
        ...
        for k, v in zip(vocab_to_keep, vocab_to_keep_frequency):
            if k.strip().strip('\n').strip('\t') != '':
