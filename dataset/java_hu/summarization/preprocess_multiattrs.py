...
from dataset.java_hu import LOGGER
    ...
    for attr in args['preprocess']['source_lang']:
        if args['preprocess']['srcdict']:
            ...
            if os.path.exists(dict_filename):
                ...
                all_filename = dict_filename.replace('java-{}'.format(portion), 'java')
...
def cli_main():
    ...
    yaml_file = os.path.join(os.path.dirname(__file__), 'config',
                             'preprocess_multiattrs_java{}.yml'.format('' if portion == 1.0 else args.portion))
