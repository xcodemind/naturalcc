...
def _evaluate(args, task, model, split):
    data_path = os.path.expanduser('~/.ncc/augmented_javascript/type_prediction/data-raw')
    # split = 'valid'
...
if __name__ == '__main__':
    ...
    args_ = Argues('javascript_eval.yml')  # train_sl
    LOGGER.info(args_)
    print('args: ', type(args_))
    # yaml_file = os.path.join('../../../naturalcodev3/run/summarization/lstm2lstm/', 'config', args_.yaml)
    yaml_file = os.path.join('../../../naturalcodev3/run/type_prediction/transformer/', 'config', args_.yaml)
