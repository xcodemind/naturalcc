...
def __terminals(ast: Dict, node_index: str, ):
    ...
    def dfs(v):
        ...
        child_nodes = utils.get_tree_children_func(v_node)
...
def __collect_sample(ast: Dict, ):
    ...
    for tree_path in tree_paths:
        ...
        new_start, new_finish = [str.lower(token) for token in utils.split_identifier(start)], \
                                [str.lower(token) for token in utils.split_identifier(finish)]
...
def ast_to_path(ast_tree: Dict):
    return __collect_sample(ast_tree)
