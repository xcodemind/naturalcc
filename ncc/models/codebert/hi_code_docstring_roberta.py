...
from ncc.modules.codebert.hi_transformer_sentence_encoder import HiTransformerSentenceEncoder
from ncc.models.hub_interface import RobertaHubInterface
from ncc import LOGGER


@register_model('hi_code_docstring_roberta')
class HiCodeDocstringRobertaModel(FairseqLanguageModel):
    ...
    def forward(self, src_tokens, src_sent_ends, doc_pad_mask, doc_pos_tok, segment_labels, features_only=False, return_all_hiddens=False, classification_head_name=None,
                 **kwargs):
        if classification_head_name is not None:
            features_only = True

        x, extra = self.decoder(src_tokens, src_sent_ends, doc_pad_mask, doc_pos_tok, segment_labels, features_only, return_all_hiddens, **kwargs)
...
class RobertaEncoder(FairseqDecoder):
    ...
    def __init__(self, args, dictionary):
        ...
        self.sentence_encoder = HiTransformerSentenceEncoder(
    ...
    def forward(self, src_tokens, src_sent_ends, doc_pad_mask, doc_pos_tok, segment_labels, features_only=False, return_all_hiddens=False,
        ...
        """
        x, extra = self.extract_features(src_tokens, src_sent_ends, doc_pad_mask, doc_pos_tok, segment_labels,
                                         return_all_hiddens=return_all_hiddens)
        if not features_only:
            x = self.output_layer(x, masked_tokens=masked_tokens)
        return x, extra

    def extract_features(self, src_tokens, src_sent_ends, doc_pad_mask, doc_pos_tok, segment_labels, return_all_hiddens=False, **unused):
        inner_states, _ = self.sentence_encoder(
            src_tokens, src_sent_ends, doc_pad_mask, doc_pos_tok, segment_labels,
