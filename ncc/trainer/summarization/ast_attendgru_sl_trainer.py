...
class AstAttendGruSLTrainer(object):
    def __init__(self, args, model, dataset ): #eval_data
        ...
        self.rnn_hidden_size_display = args['training']['rnn_hidden_size']
        self.tok_embed_size_display = args['training']['tok_embed_size']
        self.sbtao_embed_size_display = args['training']['sbtao_embed_size']

        # self.lr = 0.5
        # self.optimizer = torch.optim.Adam(model.parameters() )
        if self.args.__contains__('sl'):
            self.optimizer = getattr(torch.optim, args['sl']['optim']) \
                (self.model.parameters(), args['sl']['lr'])
    ...
    def train(self, criterion,   start_time=None,SAVE_DIR=None ):
        ...
        for epoch in range(1, 1 + self.args['training']['train_epoch']):
            ...
            model_name_prefix = "model_sl_{}_lng_{}_t{}_s{}_c{}_lr_display{}_rh{}_te{}_se{}".format(
                self.model.__class__.__name__.lower(), self.lng, self.args['dataset']['max_tok_len'],
                self.args['dataset']['max_sbtao_len'], self.args['dataset']['max_comment_len'], self.lr_display,
