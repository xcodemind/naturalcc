...
class PGTrainer(Trainer):
    ...
    def __init__(self, args: Dict, ) -> None:
        super(PGTrainer, self).__init__(args)
    ...
    def train(self, model: IModel, dataset: UnilangDataloader, lm_criterion: BaseLoss, criterion: BaseLoss, optimizer: Optimizer, reward_func='bleu',
        ...
        for epoch in range(1, 1 + model.args['training']['train_epoch']):
            model.train()
            ...
            for iteration in range(1, 1 + len(dataset['train'])):
                ...
                if model.args['pg']['rl_weight'] == 1.0:
                    LOGGER.debug('No teacher forcing.')
                    loss = pg_loss
                else:
                    sl_loss = model.train_sl(batch, lm_criterion)
                    loss = model.args['pg']['rl_weight'] * pg_loss + (1 - model.args['pg']['rl_weight']) * sl_loss
                LOGGER.debug('{} batch loss: {:.8f}'.format(self.__class__.__name__, loss.item()))
                optimizer.zero_grad()
                loss.backward()
                total_loss += loss.item()
                if model.args['pg']['max_grad_norm'] != -1:
                    nn.utils.clip_grad_norm_(model.parameters(), model.args['pg']['max_grad_norm'])
                optimizer.step()
                ...
                if iteration % model.args['training']['log_interval'] == 0 and iteration > 0:
                    LOGGER.info('Epoch: {:0>3d}/{:0>3d}, batches: {:0>3d}/{:0>3d}, avg_loss: {:.8f}; time: {}'.format(
                        epoch, model.args['training']['train_epoch'], iteration, len(dataset['train']), total_loss / iteration,
