...
class FTTrainer(Trainer):
    ...
    def __init__(self, args: Dict, ) -> None:
        super(FTTrainer, self).__init__(args)
    ...
    def pretrain(self, model: IModel, dataset: UnilangDataloader, criterion: BaseLoss, optimizer: Optimizer,
        ...
        for epoch in range(1, 1 + self.args['finetune']['pretrain_epoch']):
            model.train()
                ...
                if iteration % self.args['training']['log_interval'] == 0 and iteration > 0:
                    LOGGER.info(
                        'Epoch: {:0>3d}/{:0>3d}, batches: {:0>3d}/{:0>3d}, avg_loss: {:.6f}; lr: {:.6f}, time: {}'. \
                            format(epoch, self.args['finetune']['pretrain_epoch'], iteration, len(dataset['train']),
