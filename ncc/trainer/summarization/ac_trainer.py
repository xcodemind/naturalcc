...
class ACTrainer(Trainer):
    ...
    def __init__(self, config: Dict, ) -> None:
        super(ACTrainer, self).__init__(config)
    ...
    def train(self, model: IModel, critic: IModel, dataset: UnilangDataloader, pg_criterion, lm_criterion: BaseLoss,
        ...
        for epoch in range(1, 1 + model.config['ac']['train_epoch_ac']):
            ...
            for iteration in range(1, 1 + len(dataset['train'])):
                ...
                sample_opt = {'sample_max': 0, 'seq_length': critic.config['training']['max_predict_length']}
                ...
                if critic.config['ac']['max_grad_norm_critic'] != -1:
                    nn.utils.clip_grad_norm_(critic.parameters(), critic.config['ac']['max_grad_norm_critic'])
                ...
                if model.config['ac']['max_grad_norm'] != -1:
                    nn.utils.clip_grad_norm_(model.parameters(), model.config['ac']['max_grad_norm'])
                optimizer.step()
