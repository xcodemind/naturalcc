...
class SCTrainer(Trainer):
    ...
    def __init__(self, args: Dict, ) -> None:
        super(SCTrainer, self).__init__(args)
    ...
    def train(self, model: IModel, dataset: UnilangDataloader, lm_criterion: BaseLoss, criterion: BaseLoss,
        ...
        for epoch in range(1, 1 + model.args['training']['train_epoch']):
            ...
            for iteration in range(1, 1 + len(dataset['train'])):
                ...
                if model.args['sc']['rl_weight'] == 1.0:
                    LOGGER.debug('No teacher forcing.')
                    loss = sc_loss
                else:
                    sl_loss = model.train_sl(batch, lm_criterion)
                    loss = model.args['sc']['rl_weight'] * sc_loss + (1 - model.args['sc']['rl_weight']) * sl_loss
                LOGGER.debug('{} batch loss: {:.12f}'.format(self.__class__.__name__, loss.item()))
                optimizer.zero_grad()
                loss.backward()
                total_loss += loss.item()
                if model.args['sc']['max_grad_norm'] != -1:
                    nn.utils.clip_grad_norm_(model.parameters(), model.args['sc']['max_grad_norm'])
                optimizer.step()
