...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        if 'src_tokens' in net_input:
            ...
            src_lengths = net_input['padding_mask'].size(-1) - net_input['padding_mask'].sum(-1) if net_input[
                                                                                                        'padding_mask'] is not None else torch.tensor(
                src_tokens.size(-1))
        else:
    ...
    def _no_repeat_ngram(self, tokens, lprobs, bsz: int, beam_size: int, step: int):
        ...
        for bbsz_idx in rangeseq_gen_cls(bsz * beam_size):
            lprobs[bbsz_idx][
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def reorder_encoder_out(self, encoder_outs: Optional[List], new_order):
        """
