...
class HiTransformerSentenceEncoder(nn.Module):
    """
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, _ = layer(x, self_attn_mask=None, self_attn_padding_mask=None)
        ...
        for doc_layer in self.doc_layers:
            ...
            dropout_probability = random.uniform(0, 1)
            if not self.training or (dropout_probability > self.layerdrop):
                sent_repr, _ = doc_layer(sent_repr, self_attn_mask=None)  # doc_pad_mask
