...
def main(args, **unused_kwargs):
    ...
    for idx in range(len(query_reprs)):
        ...
        batch_query_reprs = torch.from_numpy(query_reprs[idx, :]).expand(batch_code_reprs.size(0), -1)
        similarity_scores = (batch_query_reprs * batch_code_reprs).sum(-1)
        gt_sim = similarity_scores[gt_idx]
        MRR.append(
            (1 / (similarity_scores >= gt_sim).sum().float()).item()
        )
