...
def main(parsed_args, **unused_kwargs):
    ...
    args['task']['tokens_per_sample'] -= parsed_args['eval']['context_window']
    ...
    if parsed_args['eval']['context_window'] > 0:
        dataset = LMContextWindowDataset(
            dataset=dataset,
            tokens_per_sample=args['task']['tokens_per_sample'],
            context_window=parsed_args['eval']['context_window'],
            pad_idx=task.source_dictionary.pad(),
        )
    LOGGER.info('{} {} {} examples'.format(args['task']['data'], args['dataset']['gen_subset'], len(dataset)))
    ...
    if parsed_args['eval']['remove_bpe'] is not None:
        if parsed_args['eval']['remove_bpe'] == 'sentencepiece':
            raise NotImplementedError
        else:
            bpe_cont = parsed_args['eval']['remove_bpe'].rstrip()
            bpe_toks = {
                i
    ...
    for sample in progress:
        ...
        for i, hypos_i in enumerate(hypos):
            ...
            if parsed_args['eval']['output_word_probs'] or parsed_args['eval']['output_word_stats']:
                w = ''
                word_prob = []
                is_bpe = False
                for i in range(len(tokens)):
                    ...
                    if bpe_toks is not None and w_ind in bpe_toks:
                        ...
                        is_bpe = False
                        w = ''
                if parsed_args['eval']['output_word_probs']:
                    LOGGER.info(
                        str(int(sample_id)) + " "
    ...
    if parsed_args['eval']['output_word_stats']:
...
def cli_main():
    ...
    args_ = Argues('ruby_summarize.yml')  # train_sl
    LOGGER.info(args_)
