...
def main():
    ...
    # assert model.encoder.tok_encoder.wemb.embedding.weight.size(0) == config['code_token_num']
    # assert model.decoder.wemb.embedding.weight.size(0) == config['comment_token_num']
    # assert model.decoder.linear.weight.size(0) == config['comment_token_num']
    ...
    if args.train_mode == 'train_sl':
        ...
        save_dir = os.path.join(config['dataset']['save_dir'], model.__class__.__name__.lower(),
                '-'.join(config['dataset'][args.dataset_type]['dataset_lng'])+
                                "_p{}_bi{}".format(config['dataset']['portion'],config['training']['rnn_bidirectional']), args.train_mode)
        ...
        lm_criterion = LMLoss(device=config['common']['device'] is not None, )
        optimizer = getattr(torch.optim, config['pg']['optim']) \
            (model.parameters(), config['pg']['lr'])
        save_dir = os.path.join(config['dataset']['save_dir'], model.__class__.__name__.lower(),
            '-'.join(config['dataset']['source']['dataset_lng'])+
                                "_p{}_bi{}".format(config['dataset']['portion'],config['training']['rnn_bidirectional']), args.train_mode)
        os.makedirs(save_dir, exist_ok=True)
        LOGGER.info('save_dir: {}'.format(save_dir))
        pg_trainer = PGTrainer(config)
        pg_trainer.train(model, unilang_dataset, lm_criterion, pg_criterion, optimizer,
                         config['pg']['reward_func'], SAVE_DIR=save_dir, )
        ...
        save_dir = os.path.join(config['dataset']['save_dir'], model.__class__.__name__.lower(),
            '-'.join(config['dataset']['source']['dataset_lng'])+
                                "_p{}_bi{}_rf{}".format(config['dataset']['portion'],
                                config['training']['rnn_bidirectional'],  config['sc']['reward_func']),
                                args.train_mode)
        os.makedirs(save_dir, exist_ok=True)
        LOGGER.info('save_dir: {}'.format(save_dir))
        ...
        critic = build_critic(config, MMCritic(config))
        critic_criterion = torch.nn.MSELoss(reduction='none').cuda()  # TODO: to optimized like LMLoss
        critic_optimizer = getattr(torch.optim, config['ac']['optim_critic']) \
            (critic.parameters(), config['ac']['lr_critic'])  # [args.dataset_type]
        save_dir = os.path.join(config['dataset']['save_dir'], model.__class__.__name__.lower(),
         '-'.join(config['dataset']['source']['dataset_lng'])+
                                "_p{}_bi{}".format(config['dataset']['portion'],config['training']['rnn_bidirectional']), args.train_mode)
        os.makedirs(save_dir, exist_ok=True)
        LOGGER.info('save_dir: {}'.format(save_dir))
        critic_trainer = CriticTrainer(config)
        ...
        optimizer = getattr(torch.optim, config['ac']['optim']) \
            (model.parameters(), config['ac']['lr'])
        critic = build_critic(config, MMCritic(config))
        critic_optimizer = getattr(torch.optim, config['ac']['optim_critic']) \
            (critic.parameters(), config['ac']['lr_critic'])
        pg_criterion = PGCriterion_REINFORCE().cuda()  # TODO: to optimized like LMLoss
        lm_criterion = LMLoss(device=config['common']['device'] is not None, )
        ...
        disc = build_disc(config, args.dataset_type, MMDiscriminator(config))
        disc_criterion = torch.nn.NLLLoss(reduction='sum')  # TODO: to optimized like LMLoss
        disc_optimizer = getattr(torch.optim, config['gan']['optim_disc']) \
            (disc.parameters(), config['gan']['lr_disc'])  # [args.dataset_type]
        save_dir = os.path.join(config['dataset']['save_dir'], model.__class__.__name__.lower(),
             '-'.join(config['dataset']['source']['dataset_lng'])+
                                "_p{}_bi{}".format(config['dataset']['portion'],config['training']['rnn_bidirectional']), args.train_mode)
        os.makedirs(save_dir, exist_ok=True)
        LOGGER.info('save_dir: {}'.format(save_dir))
        disc_trainer = DiscTrainer(config)
        disc_trainer.generate_training_pairs(model, unilang_dataset,
                                             filepath=os.path.join(config['gan']['comment_fake_path'],
                                                                   '{}_fake.txt'.format(
                                                                       config['dataset']['source']['dataset_lng'][0])))
        assert False
        ...
        optimizer = getattr(torch.optim, config['gan']['optim']) \
            (model.parameters(), config['gan']['lr'])
        disc = build_disc(config, args.dataset_type, MMDiscriminator(config))
        disc_optimizer = getattr(torch.optim, config['gan']['optim_disc']) \
            (disc.parameters(), config['gan']['lr_disc'])
        pg_criterion = PGCriterion_REINFORCE().cuda()  # TODO: to optimized like LMLoss
        lm_criterion = LMLoss(device=config['common']['device'] is not None, )
        ...
        optimizer = getattr(torch.optim, config['arel']['optim']) \
            (model.parameters(), config['arel']['lr'])
        reward_model = build_reward_model(config, args.dataset_type, MMRewardModel(config))
        reward_model_optimizer = getattr(torch.optim, config['arel']['optim_reward_model']) \
            (reward_model.parameters(), config['arel']['lr_reward_model'])
        pg_criterion = PGCriterion_REINFORCE().cuda()  # TODO: to optimized like LMLoss
        lm_criterion = LMLoss(device=config['common']['device'] is not None, )
        ...
        os.makedirs(save_dir, exist_ok=True)
        LOGGER.info('save_dir: {}'.format(save_dir))
        arel_trainer = ARELTrainer(config)
    ...
    #     lm_criterion = LMLoss(device=config['common']['device'] is not None, )
    #     optimizer = getattr(torch.optim, config[args.dataset_type]['optim']) \
    #         (model.parameters(), config[args.dataset_type]['lr'])
    #     meta_optimizer = getattr(torch.optim, config[args.dataset_type]['meta_optim']) \
    #         (model.parameters(), config[args.dataset_type]['meta_lr'])
