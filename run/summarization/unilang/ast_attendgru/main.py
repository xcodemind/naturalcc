...
def main():
    ...
    config, dataset, = load_config_dataset_ast_attendgru(
        args, XlangDataloader, AstAttendGruDataset, ast_attendgru_collate_fn, config=config)
    ...
    model = build_model(config,   AstAttendGru(config,unilang_dataset.token_dicts['comment']))
