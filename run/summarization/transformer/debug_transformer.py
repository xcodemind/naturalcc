...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--config", "-c", default='python_wan', type=str, help="load {language}.yml for train",
    )
    args = parser.parse_args()
    yaml_file = os.path.join(os.path.dirname(__file__), '../transformer/config', '{}.yml'.format(args.config))
    ...
    epoch = 1
    dataset = load_langpair_dataset(data_path, split,
                                    src=args['task']['source_lang'],
                                    src_dict=src_dict,
                                    tgt=args['task']['target_lang'],
                                    tgt_dict=tgt_dict,
