...
def main(parsed_args, **unused_kwargs):
    ...
    scorer = SequenceScorer(task.target_dictionary, args['dataset']['softmax_batch'])

    score_sum = 0.
    count = 0

    if args['eval']['remove_bpe'] is not None:
        if args['eval']['remove_bpe'] == 'sentencepiece':
            raise NotImplementedError
        else:
            bpe_cont = args['eval']['remove_bpe'].rstrip()
            bpe_toks = {
                i
