...
def main():
    ...
    if args_.train_mode == 'train_sl':
    ...
    elif args_.train_mode == 'train_pg':
        ...
        save_dir = os.path.join(args['dataset']['save_dir'], model.__class__.__name__.lower(),
            '-'.join(args['dataset']['source']['dataset_lng'])+
                                "_p{}_bi{}".format(args['dataset']['portion'], args['training']['rnn_bidirectional']), args_.train_mode)
        os.makedirs(save_dir, exist_ok=True)
        LOGGER.info('save_dir: {}'.format(save_dir))
    ...
    elif args_.train_mode == 'train_sc':
        pg_criterion = PGCriterion_REINFORCE().cuda()  # TODO: to optimized like LMLoss
        lm_criterion = LMLoss(device=args['common']['device'] is not None, )
    ...
    elif args_.train_mode == 'train_critic':
    ...
    elif args_.train_mode == 'train_ac':
    ...
    elif args_.train_mode == 'train_disc':
    ...
    elif args_.train_mode == 'train_gan':
    ...
    elif args_.train_mode == 'train_reward_model':
        pass

    elif args_.train_mode == 'train_arel':
        ...
        arel_trainer.train(model, reward_model, unilang_dataset, pg_criterion, lm_criterion, reward_model_criterion,
                           optimizer, reward_model_optimizer, SAVE_DIR=save_dir, )
    ...
    if args_.train_mode in [ 'train_sl' , 'train_sc' ]:
        lm_criterion = LMLoss(device=args['common']['device'] is not None, )
